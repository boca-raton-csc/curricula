...
document.addEventListener('touchstart', tryNextSlide);
document.addEventListener('mousedown', tryNextSlide);
...
function tryNextSlide(event) {
  var noSkip = ['a', 'button', 'input', 'label', 'code'];

  var tagName = event.target.tagName.toLowerCase();
  // If we do not want to skip this tag, end
  if (noSkip.indexOf(tagName) !== -1) { return; }
  // Othersie, go to the next slide
  nextSlide();
}
